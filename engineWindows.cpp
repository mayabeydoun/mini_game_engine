...
void loadRenderingConfig() {
    ...
    if (std::filesystem::exists(configFilePath)) {
        ...
        if(d.HasMember("zoom_factor") ){
            gameConfig.zoomFactor = d["zoom_factor"].GetDouble();
        }
...
///////////////////////////////
        if (!config.HasMember("initial_scene")) {
            ...
            gameConfig.initialScene = "";
        }
        if (config.HasMember("initial_scene")) {
            gameConfig.initialScene = config["initial_scene"].GetString();
        }
        
        if (!config.HasMember("game_title")) {
            std::cout << "error: game_title unspecified";
            gameConfig.gameTitle = "";
        }
        if (config.HasMember("game_title")) {
            gameConfig.gameTitle = config["game_title"].GetString();
        }
...
std::string fetchInitialScene() {
    ...
    return gameConfig.initialScene;
}

std::string fetchGameTitle() {

    return  gameConfig.gameTitle;
}
...
void drawPlayerScore(SDL_Renderer* renderer,TTF_Font* font, int score) {
    ...
    TTF_Init();
    std::string fontPath = introScreenState.fontPath;
    font = TTF_OpenFont(introScreenState.fontPath.c_str(), 16);
...
bool renderMapAroundPlayer(glm::ivec2& player_position, SDL_Renderer* renderer ) {
    ...
    glm::vec2 cameraPosition = { (gameConfig.cam_offset_x * 100) ,
                                 (gameConfig.cam_offset_y * 100)  };
    ...
    for (auto& actor : actors_custom) {
        ...
        if (actor.name == "player") {
            ...
            float halfWidth = render_width / 2.0f;
            float halfHeight = render_height / 2.0f;

            // Recalculate camera position based on the zoom factor to ensure the scene remains centered.
            cameraPosition = glm::vec2(
                (player_position.x + gameConfig.cam_offset_x * 100) ,
                (player_position.y + gameConfig.cam_offset_y * 100)
            );
            break;
        }
    }
    ...
    //std::cout << " cam pos " << cameraPosition.x << " " << cameraPosition.y <<std::endl;
    ...
    // First, set the global render scale for zooming.
    // If you decide to manually scale everything, you can comment this out and adjust calculations accordingly.
    SDL_RenderSetScale(renderer, gameConfig.zoomFactor, gameConfig.zoomFactor);
    ...
    for (auto& actor : actors_custom) {
        ...
        // Calculate the original (unscaled) width and height based on the actor's scale.
        int originalWidth = static_cast<int>(std::round(textureWidth * std::abs(actor.transform_scale.x)));
        int originalHeight = static_cast<int>(std::round(textureHeight * std::abs(actor.transform_scale.y)));

        // Calculate pivot offset based on the original dimensions, not considering zoom here.
        glm::vec2 pivotOffset = {
            actor.view_pivot_offset.x != 0 ? actor.view_pivot_offset.x * std::abs(actor.transform_scale.x) : static_cast<float>(originalWidth) / 2,
            actor.view_pivot_offset.y != 0 ? actor.view_pivot_offset.y * std::abs(actor.transform_scale.y) : static_cast<float>(originalHeight) / 2,
        };

        SDL_Point pivot = {static_cast<int>(pivotOffset.x), static_cast<int>(pivotOffset.y)};



        glm::vec2 focalPoint = player_position;

        float centerX = render_width / 2.0f;
        float centerY = render_height / 2.0f;

      
        cameraPosition.x = focalPoint.x - (centerX * ((1 + gameConfig.zoomFactor) - gameConfig.zoomFactor)) + (gameConfig.cam_offset_x * 100);
        cameraPosition.y = focalPoint.y - (centerY * ((1 + gameConfig.zoomFactor) - gameConfig.zoomFactor)) + (gameConfig.cam_offset_y * 100);


        glm::vec2 screenPosition = {
            actor.transform_position.x - cameraPosition.x + (render_width / 2) - pivotOffset.x,
            actor.transform_position.y - cameraPosition.y + (render_height / 2) - pivotOffset.y,
        };
        ...
        // Render the actor with the calculated parameters.
